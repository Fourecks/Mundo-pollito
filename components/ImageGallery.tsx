...
const ImageGallery: React.FC<ImageGalleryProps> = ({ images, onAddImages, onDeleteImage, isMobile = false, isSignedIn, onAuthClick, isGapiReady, isLoading, currentUser }) => {
  const [viewingIndex, setViewingIndex] = useState<number | null>(null);
  const [isDraggingOver, setIsDraggingOver] = useState(false);
  const [activeTab, setActiveTab] = useState<'gallery' | 'games'>('gallery');
  ...
  return (
    <div className="flex flex-col h-full w-full">
        <header className="flex-shrink-0 p-1 bg-black/5 dark:bg-black/20 border-b border-secondary-light/30 dark:border-gray-700/50">
            <div className="flex items-center">
                <button onClick={() => setActiveTab('gallery')} className={`w-1/2 py-1.5 text-sm font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 ${activeTab === 'gallery' ? 'bg-white dark:bg-gray-600 shadow text-primary-dark dark:text-gray-100' : 'text-gray-600 dark:text-gray-300 hover:bg-white/50 dark:hover:bg-black/20'}`}><GalleryIcon /> Galería</button>
                <button onClick={() => setActiveTab('games')} className={`w-1/2 py-1.5 text-sm font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 ${activeTab === 'games' ? 'bg-white dark:bg-gray-600 shadow text-primary-dark dark:text-gray-100' : 'text-gray-600 dark:text-gray-300 hover:bg-white/50 dark:hover:bg-black/20'}`}><GameIcon /> Juegos</button>
            </div>
        </header>

        <div className="flex-grow overflow-hidden">
            {activeTab === 'gallery' ? renderGalleryContent() : <GamesHub galleryImages={images} isMobile={isMobile} currentUser={currentUser} />}
        </div>
