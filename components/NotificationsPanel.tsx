      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-black/30 backdrop-blur-[3px] z-[90000]"
          />
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 h-full w-full md:max-w-3xl bg-[#fafafa] dark:bg-[#0c0d0e] shadow-2xl z-[90001] border-l border-gray-200/80 dark:border-gray-800/80 flex flex-col overflow-hidden"
          >
            {/* Drawer Header */}
            <header className="flex-shrink-0 px-6 py-5 border-b border-gray-150 dark:border-gray-800/80 bg-white dark:bg-[#111213] flex items-center justify-between">
              <div>
                <h3 className="font-bold text-sm tracking-tight text-gray-900 dark:text-gray-100 uppercase">
                  Buzón de Actividad
                </h3>
                <p className="text-xs text-gray-400 mt-0.5">
                  Invitaciones y alertas del sistema de manera directa
                </p>
              </div>
              <button 
                onClick={onClose} 
                className="p-1.5 rounded hover:bg-gray-150 dark:hover:bg-gray-800/80 text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 transition-colors"
            ...
            {/* Main content body - grid containing side-by-side cards */}
            <div className="flex-grow p-6 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                
                {/* COLUMN 1: SYSTEM ANNOUNCEMENTS */}
                <div className="bg-white dark:bg-[#111213] rounded-xl border border-gray-200/80 dark:border-gray-800/80 shadow-sm flex flex-col overflow-hidden">
                  <div className="px-4 py-3.5 border-b border-gray-150 dark:border-gray-800/80 bg-gray-50/50 dark:bg-[#141517] flex items-center justify-between">
                    ...
                    {unreadAnnouncementsCount > 0 && (
                      <button
                        ...
                        className="text-[10px] font-bold text-gray-900 hover:text-gray-600 dark:text-gray-200 dark:hover:text-gray-400 flex items-center gap-1 transition-colors border border-gray-200 dark:border-gray-700 px-2 py-0.5 rounded"
                      >
                  ...
                  <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar divide-y divide-gray-100 dark:divide-gray-800/50">
                ...
                {/* COLUMN 2: PROJECT REQUESTS / INVITATIONS */}
                <div className="bg-white dark:bg-[#111213] rounded-xl border border-gray-200/80 dark:border-gray-800/80 shadow-sm flex flex-col overflow-hidden">
                  <div className="px-4 py-3.5 border-b border-gray-150 dark:border-gray-800/80 bg-gray-50/50 dark:bg-[#141517] flex items-center justify-between">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                      Solicitudes ({pendingInvitations.length})
                    </span>
                  </div>
                  ...
                  <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
                    {pendingInvitations.length === 0 ? (
                      <div className="text-center py-12 px-4">
                        <Mail className="w-6 h-6 text-gray-300 dark:text-gray-600 mx-auto mb-2" />
                        <p className="text-xs font-semibold text-gray-600 dark:text-gray-400">No hay solicitudes pendientes</p>
                        <p className="text-[10px] text-gray-400 mt-0.5">Las invitaciones a tableros aparecerán en este bloque.</p>
                      </div>
                    ) : (
                      pendingInvitations.map((inv) => (
                        <div
                          key={inv.id}
                          className="p-3.5 rounded-lg border border-gray-200 dark:border-gray-800/80 bg-gray-50/40 dark:bg-black/10 flex flex-col gap-3 transition-all"
                        >
                          <div className="flex items-start gap-3">
                            <div
                              className="w-8 h-8 rounded text-sm flex items-center justify-center font-bold text-gray-800 dark:text-gray-150 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shrink-0 shadow-xs"
                            >
                              {inv.project_emoji || '📁'}
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-gray-400 dark:text-gray-400 mt-0.5">
                                Invitado por: <span className="font-semibold text-gray-700 dark:text-gray-300">{inv.inviter_name || inv.inviter_email || 'Colaborador'}</span>
                              </p>
                            </div>
                          </div>
